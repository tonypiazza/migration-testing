...
output "privatelink_service_name" {
  description = "PrivateLink VPC endpoint service name to give the migration consumer as target_connectivity.service_attachment. Empty when enable_psc = false."
  value       = var.enable_psc ? aws_vpc_endpoint_service.os[0].service_name : ""
}
